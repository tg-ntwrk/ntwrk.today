...
Роутер должен выполнять роль DHCP сервера или DHCP Relay. Возможен старт сессии как по `Unclassified MAC Address`, так и по `DHCP packet`.
...
1. Если в шасси установлена комбинация ESP-20 + SIP-40, то SIP-40 работает как SIP-10. То есть можно максимум установить одну 10G SPA без переподписки.
