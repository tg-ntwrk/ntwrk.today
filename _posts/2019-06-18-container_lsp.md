...
- ECMP - sub-LSPs используются для балансировки. Каждый sub-LSP может использовать отличный путь к тому же egress маршрутизатору. Доступны для выбора разные методы балансировки по LSP:
  - random;
  - least-fill;
  - most-fill.
        ...
        protocols {
            mpls {
                container-label-switched-path <*> {
                    ...
                    splitting-merging {
                        minimum-member-lsps 4; ## так как не задан maximum-member-lsps, то значение будет по умолчанию 64, что много
