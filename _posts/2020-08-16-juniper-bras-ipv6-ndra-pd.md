...
IPv6 хоть и не так быстро как хотелось, но расширяет свое присутствие в Интернет, да и у меня лет 10 уж точно на нем маленький пунктик - добавляю где только возможно. Являясь администратором довольно большой клиентской базы, подключенной по GPON, довольно рано озаботился его внедрением. Года два ушло на бокс по переписке с производителем GPON оборудования, и после десятка апдейтов прошивок OLT и ONT, тестирования конфигурации и развертывания, около 60 тысяч клиентов получили доступ в Интернет и по IPv6, резко подняв нас как провайдера и нашу маленькую, но гордую страну в соответствующих рейтингах.
    ...
    ae1 {
        ...
        unit 106 {
            ...
            family inet {
                ...
                rpf-check fail-filter RPF-ALLOW-DHCP;
                unnumbered-address lo0.0 preferred-source-address 1.2.0.1;
            }
            family inet6 {
                unnumbered-address lo0.0 preferred-source-address 2a02:1:1:a::1;
            }
        }
    }
    lo0 {
...
access {
    profile access {
        ...
        radius {
            authentication-server 1.2.3.2;
            accounting-server 1.2.3.2;
            options {
    ...
    address-assignment {
        ...
        pool v6-prefix-wan {
            family inet6 {
                prefix 2a02:1:1:a::/64;
                range R1 {
                    low 2a02:1:1:a::2/128;
                    high 2a02:1:1:a::ffff:ffff/128;
                }
                dhcp-attributes {
                    domain-name xxxx;
                    dns-server {
