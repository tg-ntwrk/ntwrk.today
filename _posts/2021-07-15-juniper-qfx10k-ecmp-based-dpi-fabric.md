...
Открываем нашу [старую целевую в PDF](/images/2021-07-15-juniper-qfx10k-ecmp-based-dpi-fabric_old.pdf) и смотрим детальнее, как это выглядит:
...
![ecmp-v3_link-failure](/images/2021-07-15-juniper-qfx10k-ecmp-based-dpi-fabric_ecmp-v3_link-failure.png)<br/>
...
![scale-fabric-normalized](/images/2021-07-15-juniper-qfx10k-ecmp-based-dpi-fabric_scale-fabric-normalized.png)<br/>
...
Кроме того, для увеличения скорости сходимости сети, данный уровень может использовать ободные туннели через edge/spine, чего раньше мы не могли себе позволить с leaf коммутаторами.
...
Минусы:
* **Данная схема стоит дороже**, так как требуются еще коммутаторы (но это могут быть не дорогие коммутаторы без MPLS, VXLAN или других сложных в реализации технологий);
...
*В конкретном примере мы рассмотрим конфигурацию edge, для spine и tfs коммутаторов конфигурация не будет сильно отличаться*
...
* edge/spine коммутаторы порадовали асимметричным прохождением трафика - открыт кейс производителю, проблема найдена и подтверждена, ждем устранения в следующих релизах ПО (на текущий момент есть конфигурационный workaround в нашем кейсе);
...
### Подкаст про все вышеперечисленное, в контексте данной заметки:
